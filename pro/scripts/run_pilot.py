...
def run_pilot() -> None:
    ...
    print("  PILOT RUN — Hetauda Cement + Bhairahawa Brick")
    ...
    # 1. CEMENT BASELINE
    # ====================================================================
    print("\n[1] CEMENT BASELINE — Hetauda Cement Industries Ltd")
    hetauda = CementPlant(
    ...
    t2 = calculate_cement_tier2(hetauda, ef)
    t3 = calculate_cement_tier3(hetauda, ef)
    ...
    mc = monte_carlo_cement(hetauda, ef, spec, n_samples=5000)
    ...
    p_kiln = KilnParameters(raw_meal_throughput_t_h=hetauda.clinker_production_t * 1.55 / 8760,
    ...
    dxf = generate_dxf_kiln(out_path=out_dir / "hetauda_kiln.dxf")
    svg = generate_svg_kiln(out_path=out_dir / "hetauda_kiln.svg")
    macro = generate_freecad_macro(out_path=out_dir / "hetauda_kiln.FCMacro")
    pfd_c = generate_pfd_cement(out_dir / "hetauda_pfd.png")
    pfd_b = generate_pfd_brick(out_dir / "bhairahawa_pfd.png", kiln_type="clamp_traditional")
    pid = generate_pid_cement(out_dir / "hetauda_pid.png")
    ...
    # ====================================================================
    print("\n[12] STANDARDS COMPLIANCE")
    iso1 = check_iso_14064_part1(plant=hetauda, cement_result=t2)
    iso2 = check_iso_14064_part2(verification_done=True, validation_done=True)
    iso3 = check_iso_14064_part3()
    iso5k = check_iso_50001(hetauda.name)
    iso14k = check_iso_14001(hetauda.name)
    tcfd = generate_tcfd_report(plant=hetauda, cement_result=t2)
    ...
    gcca = calculate_gcca_kpis(hetauda, t2, ef)
    ...
    md = build_token_metadata(
        project_name="Hetauda Decarb", vintage_year=2026,
    ...
    # ====================================================================
    print("\n[14] PDF REPORTS")
    pdf1 = generate_executive_summary(hetauda, t2, out_dir / "hetauda_executive.pdf")
    pdf2 = generate_verra_monitoring_report(hetauda, t2, project_em, "Hetauda Decarb",
                                             out_dir / "hetauda_verra.pdf")
    pdf3 = generate_iso_14064_report(hetauda, t2, iso1, out_dir / "hetauda_iso.pdf")
    pdf4 = generate_tcfd_pdf_report(hetauda, t2, tcfd, out_dir / "hetauda_tcfd.pdf")
    ...
    out_json = out_dir / "hetauda_pilot_results.json"
    ...
    print("  Production-ready for pilot deployment at Hetauda Cement + Bhairahawa Brick")
    print("=" * 80)
