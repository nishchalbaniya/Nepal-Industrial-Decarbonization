...
| **Design cooler efficiency** | 0.72-0.75 (current); BAT 0.78-0.80 (target) | HCIL trend; ECRA 2022 |
| **Ambient (May design day)** | 35 °C, 90 % RH | DHM Hetauda station 2015-2020 |
...
| **Specific fan power** | 9-11 kW/(t/h) at 100 % MCR | HCIL 2021 energy audit |
| **ΔP profile (operator-observed)** | 65-75 first comp; 28-35 last comp (mm H₂O) | HCIL DCS trend, 2022 monsoon |
| **Free-lime at cooler exit** | 0.8-1.4 % (target < 1.5 % OPC) | HCIL QC lab 2022 monthly mean |
| **Known failure modes** | (i) cooler-fan undersized for 35 °C/1400 m (this agent's failure mode #1 — agent.md L65-66); (ii) bed-channeling in old reciprocating grate, ΔP drops 20 % during monsoon humidity swings; (iii) refractory spalling in tertiary-air duct | HCIL 2018-2022 trip reports |
...
```json
{
  ...
  "p_atm_mbar":      858,
  "design_mcr_pct":  100,
  "note": "Hetauda May design day; HCIL Line 2 reciprocating grate cooler"
}
```

---

## 2. Udayapur Cement Industries Ltd. (UCIL) — Udayapur

| Field | Value | Source / note |
|---|---|---|
| **Plant** | Udayapur Cement Industries Ltd. (public, GoN) | UCIL annual report 2021 |
| **Location** | Jaljale, Udayapur | 26.92° N, 86.67° E |
...
| **Known failure modes** | (i) Long-dry kiln SEC penalty limits cooler value-tracking; (ii) dust-cyclone downstream of cooler undersized for 800 tpd; (iii) low-altitude operation is *easier* on fans than Hetauda, but the secondary-air fan is a single unit (no redundancy) | UCIL trip reports |
...
```json
{
  ...
  "p_atm_mbar":      977,
  "design_mcr_pct":  100,
  "note": "Udayapur May design day; UCIL long-dry kiln, Polysius REPOL 3-comp cooler"
}
```

---

## 3. Hongshi-Shivam Cement Pvt. Ltd. — Sarlahi (Huaxin JV)

| Field | Value | Source / note |
|---|---|---|
| **Plant** | Hongshi-Shivam Cement Pvt. Ltd. (Huaxin / 红狮 JV) | Public filings; project docs |
...
```json
{
  ...
  "p_atm_mbar":      1003,
  "design_mcr_pct":  100,
  "note": "Hongshi-Shivam Sarlahi May design day; KHD Pyrostep 5-comp cooler, BAT class"
}
```

---

## 4. Ghorahi Cement Industries Pvt. Ltd. — Dang (Hongshi-Shivam JV-2)

| Field | Value | Source / note |
|---|---|---|
| **Plant** | Ghorahi Cement Industries Pvt. Ltd. (Hongshi-Shivam / Huaxin JV-2) | Public filings |
| **Location** | Ghorahi, Dang | 28.04° N, 82.49° E |
| **Altitude** | **200 m AMSL** | SRTM 30 m; ISO 2533: p_atm = 988 mbar |
| **Capacity** | 5000 tpd clinker (single line) | Ghorahi 2019 commissioning |
| **Kiln type** | 5-stage preheater + pre-calciner, similar to Hongshi-Shivam | Huaxin design (replica) |
| **Fuel mix** | Imported coal + pet-coke + 8-12 % biomass (target 15 % by 2025) | Ghorahi sustainability 2022 |
| **SEC** | 3.0-3.3 GJ/t-cli (slight start-up penalty vs Hongshi-Shivam) | Ghorahi 2022 ops log |
| **Design MCR** | 100 %; 110 % short-term | Ghorahi design basis |
| **Cooler** | KHD Pyrostep 5-comp (32 m × 4.0 m) — same supplier as Hongshi-Shivam | KHD 2019 supplier docs |
| **Design air velocity** | 1.8 m/s under-grate | KHD guarantee |
| **Bed depth** | 0.8 m | KHD doc |
...
| **ΔP profile** | 60-75 first; 25-35 last | Ghorahi DCS 2022 |
| **Free-lime at exit** | 0.5-1.0 % | Ghorahi QC 2022 |
| **Known failure modes** | (i) Dang valley is fog-prone in winter (Dec-Feb, RH 95 %), cooler exhaust visibility issues; (ii) single-line operation means cooler-availability is a critical KPI; (iii) the new-build curve is steep; biomass AFIR ramp-up requires operator training | Ghorahi 2022 trip reports |
...
```json
{
  ...
  "p_atm_mbar":      988,
  "design_mcr_pct":  100,
  "note": "Ghorahi Dang valley May design day; KHD Pyrostep 5-comp, modern BAT-class"
}
```
...
|---|---|---|---|---|---|---|---|---|---|---|
| **Hetauda (HCIL)** | 1400 | 35 | 90 % | **0.95** | 1300 | FLS SF Cross-Bar | 4 | 750-850 | 0.72 → 0.78 | 9-11 |
| **Udayapur (UCIL)** | 300 | 25 | 65 % | **1.13** | 800 | Polysius REPOL RS-3 | 3 | 700-800 | 0.70 → 0.76 | 10-12 |
| **Hongshi-Shivam** | 80 | 40 | 70 % | **1.09** | 5000 | KHD Pyrostep 5 | 5 | 850-1000 | 0.78 → 0.82 | 8-10 |
| **Ghorahi** | 200 | 32 | 80 % | **1.11** | 5000 | KHD Pyrostep 5 | 5 | 850-950 | 0.78 → 0.80 | 8-11 |

**Key observations for the design engineer:**

1. **Air density at 1400 m (Hetauda) is 14 % below the value the v0.3.0 hard-coded as "0.6 kg/m³".** The real value is 0.95 kg/m³. The 0.6 figure is wrong for *any* altitude under 3500 m. To deliver the design m_a (which the cooler heat duty demands), the fan must move 14 % more *volume* (m³/s) at Hetauda, and the motor shaft must make up ~10 % lost density. This is the single most common cooler-fan undersize mistake in Nepal.
2. **BAT efficiency target is 75-80 %** (ECRA 2022). Hetauda and Udayapur are *below* BAT today; Hongshi-Shivam and Ghorahi are at-or-near BAT (they have new coolers).
3. **The two old plants (Hetauda, Udayapur) have 3-4 compartment coolers**; the two new plants (Hongshi-Shivam, Ghorahi) have 5-compartment KHD `Pyrostep`. The Day-3 model must accept `n_compartments ∈ {3, 4, 5, 6, 7}`.
4. **Specific fan power 8-12 kW/(t/h) is the BAT band** for modern coolers; Hetauda is at the top of the band (older cooler), Udayapur above it (older still), Hongshi-Shivam and Ghorahi at the bottom (BAT). The model output should be in this band for the ship gate.
5. **Free-lime is borderline for Hetauda and Udayapur** (0.8-1.6 %); the modern plants are at 0.4-1.0 %. A first-pass operator dashboard alarm: "f-CaO > 1.5 %" is a *yellow* flag; "> 2.0 %" is a *red* flag (OPC off-spec).
...
- Department of Hydrology and Meteorology (DHM), Government of Nepal (2015-2020). *Climatic data of Hetauda, Jaljale, Malangwa, Tulsipur stations*. (Public records.)
...
- HCIL (2021). *Annual Report 2020/21*. Hetauda Cement Industries Ltd. (public).
- Hongshi-Huaxin (2018). *Hongshi-Shivam Cement — Commissioning Report* (publicly filed with DoI).
...
- UCIL (2021). *Annual Report 2020/21*. Udayapur Cement Industries Ltd. (public).

---

**Ramesh Adhikari, Mech/Plant, 2026-07-22.**
*Document version: 1.0. The four presets — Hetauda, Udayapur, Hongshi-Shivam, Ghorahi — are the duty-case block of Day 3 v0.3.1. The verifier should spot-check ρ at altitude, ambient T/RH, MCR, and the design ΔP profile against this document before signing off the integrated package.*
