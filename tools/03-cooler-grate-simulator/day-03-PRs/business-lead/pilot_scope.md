# Pilot Scope — 90-day Pilot at PlantA Industries Ltd (NIDC)

> **Owner:** Priya Karki, Business Lead
> **Date:** 2026-07-22
> **Audience:** NIDC plant management + internal team (Aanya, Ramesh, Hiro, Maya, Kabita, James, Sofia)
...
**NIDC PlantA** operates a wet-process kiln with a cooler that was not designed for the site's altitude. The plant has historically under-performed on cooler heat recovery because:
...
**Skin in the game from NIDC**: 1 dedicated plant operator (≥25 % of their time), 1 plant engineer (≥10 %), real-time DCS export of cooler-relevant tags, monthly raw-mill and kiln-coal samples for proximate/ultimate analysis, monthly clinker free-lime (f-CaO) at cooler exit.
...
**Pass / fail**: Pass = proceed to Day 90. Fail = pilot extends by 30 days at no additional cost to NIDC, or terminates with 50 % refund at NIDC's option.
...
- Expected payback in months at NIDC's current fuel cost
...
- The plant's engineering team has reviewed the report and signed off on at least 3 of the opportunities as technically actionable within NIDC's operating envelope.

**Per Ramesh's negotiation-room answer (2026-07-22)**: the 90-day target is a *Phase-1 stretch*. The realistic SEC-reduction target for cooler tuning alone is **1.5 % at PlantA in 6 months** (180 days), not 90 days. The pilot's Day-90 deliverable is therefore *opportunity identification with engineering signature*, not *realized SEC reduction*. The 6-month milestone is the first realized-savings checkpoint in the 3-year license.

**Pass / fail**: Pass = the renewal path opens (see below). Fail = we have learned what we need to learn; we keep the report; the pilot closes; NIDC keeps the calibrated model and the 90 days of baseline data; no additional fee.

**Why "signed by a chartered engineer"**: this is the proof a plant CFO and a plant COO will both read. The CFO buys the SEC delta. The COO buys the engineering signature. Aanya, Ramesh — please confirm you are willing to put your PE/Chartered signature on the Day 90 report for the right plant. (If the answer is "no," the pilot cannot be a PlantA pilot at this stage; we go to a plant where the answer is "yes.")
...
- **NIDC signs a 3-year license** at $50K–75K/yr (depending on plant size, see `pricing_math.md`).
...
- **Year 3**: same, plus the Verra PDD package (if NIDC chooses to issue credits; co-developed with the carbon-markets-expert, James, and the env-eng-permitting, Kabita).
- **60-day renewal notice** at the end of each year. **No auto-renew** without a written kickoff of the next-year scope. The data-ownership clause is in the contract: NIDC owns the data; we own the calibrated model. We do not train on NIDC's data without written consent.
- **Exclusivity**: 12-month exclusivity on NIDC's plant in the Nepali market. We do not sell the same calibrated model to another Nepali plant during the 12-month exclusivity window.
- **Credit ownership (per James, carbon-markets-expert, negotiation room 2026-07-22)**: if the cooler-tuning opportunities realized in years 1–3 of the license lead to a Verra / Gold Standard / Article 6.4 issuance, **the credits are owned by NIDC** (or shared per a pre-agreed split documented in the PDD per VCS Program Guide v4.5 §3.5 and ICVCM Core Carbon Principles 2023 §3). We do not retire NIDC's credits against a third-party offsetting commitment. The PDD and monitoring plan will document this explicitly. The failure mode to avoid is "developer retires first, plant never gets the retirement receipt" — James has seen this in three Toucan-pooled issuances in 2023.

If the Day 90 report identifies < 1 % SEC reduction opportunity (the plant is already at BAT), the renewal path does not open. The pilot closes. NIDC keeps the calibrated model and the 90 days of baseline data. We refund 25 % of the pilot fee as a thank-you for the time.
...
- Travel (3 site visits, Kathmandu → PlantA) = $4K
...
## Why PlantA, not plantc or PlantD

- **PlantA** is a legacy plant with 1–3 % SEC reduction on the table. The plant is the right size (3000 tpd class) and the right altitude to expose the fan-power / duty-case issue (Ramesh §5). NIDC has historical operating data and a control room that exports DCS. The plant is a 4-hour drive from Kathmandu; site visits are cheap.
- **PlantB** is similar (UCIL), at a lower altitude (~300 m). The pitch is similar but the duty case is easier. Pilot #2, not pilot #1.
- **plantc** is a new plant (post-2018). The easy wins are gone; the pitch is "0.5–1 % SEC reduction on a Chinese-built plant, plus the data quality for future credits." Different price, different pitch. Pilot #3, not pilot #1.
- **PlantD** is mid-life, 1–2 % SEC reduction. Good pilot #4.

We do pilot #1 at PlantA. We do not start with plantc. The first customer is the cheapest to acquire and the most expensive to support; we want a customer who will *use* the product, not a customer who will *evaluate* it for 6 months and decide.

## What we need from the team before we go to NIDC

- **Aanya**: confirm the 1–4 % range is the honest engineering claim for a PlantA-class plant.
- **Ramesh**: confirm the PlantA 1400 m / 35 °C / 90 % RH duty case is the right design day for the pilot.
- **Hiro**: confirm the Day 30 / 60 / 90 success metrics are measurable on NIDC's actual DCS export.
...
If any of the seven cannot confirm, we do not go to NIDC with this scope. We revise and re-post.
...
2. **Ramesh's Day 3 review** (`reviews/DAY-03-RAMESH-REVIEW.md`) — PlantA 1400 m / 35 °C / 90 % RH duty case, first-law check, compartment design.
