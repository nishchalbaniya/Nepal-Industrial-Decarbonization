...
# v0.3.2: try a relative import first (production), fall back to the
# bare module name (smoke / ad-hoc test path, when src/ is on sys.path
# rather than src/nepal_cooler_sim/). The fallback is the same pattern
# Maya used in cooler_types.py and keeps Aanya's PR importable either
# way without a conftest hack.
try:
    ...
    from cooler_ode import (  # type: ignore[no-redef]
        AIR_CP_J_KG_K,
        AIR_K_W_M_K,
        AIR_VISCOSITY_PA_S,
        HOOD_RADIATION_MARGIN_K,
