"""Day 4 — Generate a synthetic PlantA plant-data CSV.

PURPOSE
-------
Day 4 (cooler calibration to PlantA plant data) needs one operating
...
with the v0.3.2 model, but it is NOT real PlantA plant data. Real
plant data must replace this before any Verra PDD or CBAM XML is
filed.

WHAT THIS IS NOT
----------------
- Not real. A real PlantA operating shift from the DCS export has
...
-----
    ...
    -> writes data/synthetic_planta_shift_4h.csv (240 rows)
...
# Operating point: a "design-duty" PlantA that v0.3.2 is calibrated TOWARD,
...
DESIGN_DUTY = dict(
    ...
    # Coal rate 3.6 kg/s (PlantA spec)
    coal_rate_kg_s = 3.6,
...
def main() -> None:
    ...
    out = here / "data" / "synthetic_planta_shift_4h.csv"
    ...
    print("  PlantA plant shift. Use it to develop the Day 4 calibration")
